...
export class SPKFileUpload {
   ...
   */
  private async uploadBatch(files: File[], options: UploadOptions): Promise<BatchUploadResult> {
    // Ensure account has registered public key
    await this.account.registerPublicKey();
    ...
    if (totalBrocaCost > availableBroca) {
      throw new Error(
        `Insufficient BROCA. Required: ${totalBrocaCost}, Available: ${availableBroca}`
      );
    }
    ...
    for (let i = 0; i < filesWithMetadata.length; i++) {
      ...
      await this.uploadToIPFS(
        uploadFile,
        batchContract.i,
        metadata?.onProgress || options.onProgress,
        {
          ...batchContract,
          cid: cid,
  ...
  private validateMetadata(files: File[], metadata: FileMetadataItem[]): void {
    for (const meta of metadata) {
      if (meta.FileIndex < 0 || meta.FileIndex >= files.length) {
        throw new Error(
          `Invalid FileIndex ${meta.FileIndex}. Must be between 0 and ${files.length - 1}`
        );
      }
    }
  }
  ...
  private async encrypt(file: File, recipients: string[]): Promise<any> {
    try {
      ...
      return {
        ...
        encryptedKeys: result.metadata.encryptedKeys.reduce(
          (acc, item) => {
      ...
      return {
        encryptedData: data,
        encryptedKeys: recipients.reduce(
          (acc, recipient) => {
   ...
   */
  private async uploadSingleNodeFile(file: any, options: UploadOptions): Promise<UploadResult> {
    // Ensure account has registered public key
    await this.account.registerPublicKey();
   ...
   */
  private async uploadNodeBatch(files: any[], options: UploadOptions): Promise<BatchUploadResult> {
    // Ensure account has registered public key
    await this.account.registerPublicKey();
    ...
    for (let i = 0; i < filesWithMetadata.length; i++) {
      ...
      const fileProgress = metadata?.onProgress || options.onProgress;
      console.log(
        `[SPK-JS] Uploading file with CID ${cid}, uploadFile is encrypted: ${uploadFile !== file}`
      );
      ...
      for (let i = 0; i < chunks; i++) {
        ...
        try {
          await this.uploadNodeChunk(
            ...
            (chunkProgress) => {
              const totalProgress =
                ((uploaded + (chunkProgress * chunkBuffer.length) / 100) / fileSize) * 100;
              onProgress?.(Math.round(totalProgress));
            },
            cid
          );
        ...
        if (response.status === 409 && retryCount === 0) {
          try {
            ...
            if (newStart > start) {
              ...
              console.log(
                `[SPK-JS NODE] Adjusting chunk: skipping ${skipBytes} bytes, sending ${adjustedBuffer.length} bytes from position ${newStart}`
              );
