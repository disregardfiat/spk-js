 ...
 */
export const PRESET_FOLDER_INDICES: Record<string, string> = Object.entries(PRESET_FOLDERS).reduce(
  (acc, [name, idx]) => {
    acc[idx] = name;
    return acc;
  },
  ...
  for (let i = 0; i < sortedCids.length; i++) {
    ...
    if (baseIndex + 3 < parts.length) {
      ...
      if (folderMap.has(pathIndex)) {
        ...
        metadata.fullPath = folder.fullPath
          ? `${folder.fullPath}/${name}${ext ? '.' + ext : ''}`
          : `${name}${ext ? '.' + ext : ''}`;
      } else {
        metadata.folder = '';
...
export function buildMetadataString(
  ...
  const fileEntries = sortedCids
    .map((cid) => {
      ...
      return [meta.name || '', extAndPath, meta.thumb || '', flagsData].join(',');
    })
    .join(',');
